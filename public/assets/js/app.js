// ============================================================================
// public/assets/js/app.js — MOTOR DE LA WEB (ACTUALIZADO v3)
// ✅ Lupa inteligente: busca en PAQUETES + PLAYAS + TRASLADOS
// ✅ Rutas locales ELIMINADAS: solo URLs completas (http/https)
// ✅ Sin fallback local: si no hay URL válida → emoji + degradado
// ============================================================================
'use strict';

// ── Utilidades base ─────────────────────────────────────────────────────────
// $ y on ya están declaradas en api.js (carga primero)
// LOGO_URL, COMPANY_WA, COMPANY_MAIL, BACKEND_ENABLED están definidas en data.js
...
// ── 🔒 URLs SEGURAS: solo se acepta http/https. Todo lo demás se descarta ───
const isFullUrl = (u) => typeof u === 'string' && /^https?:\/\//i.test(u.trim());
const safeImg = (u) => (isFullUrl(u) ? u.trim() : '');
...
const DEFAULT_BEACHES = [
  ...
  { id: 'playa-contadora', name: 'Isla Contadora', zone: 'Archipiélago de las Perlas', emoji: '🛥️', price: 95, duration: '🕒 Día completo (ferry)', img: 'https://images.unsplash.com/photo-1476673160081-cf065607f449?q=80&w=1200&auto=format&fit=crop', desc: 'La joya de las Perlas: 13 playas de arena perlada, aguas calmas y atardeceres de postal a 45 min en ferry.' },
...
const saveSocial = () => console.log('Redes actualizadas:', SOCIAL);
...
// ── 🔒 SANITIZADOR DE URLs: limpia los datos al cargar/guardar ──────────────
function sanitizeUrls() {
  const cleanList = (arr) => (arr || []).forEach(o => {
    if (!o || typeof o !== 'object') return;
    if ('img' in o) o.img = safeImg(o.img);
    if (Array.isArray(o.imgs)) o.imgs = o.imgs.map(safeImg).filter(Boolean);
    if ('video' in o && o.video && !isFullUrl(o.video)) o.video = '';
  });
  cleanList(TOURS); cleanList(BEACHES); cleanList(SERVICES); cleanList(ABOUT_BLOCKS);
}

// ── 🔍 LUPA: utilidades de coincidencia (paquetes, playas, traslados) ───────
let globalSearchQuery = '';
const normQ = (s) => (s || '').toLowerCase();
const matchQ = (q, fields) => fields.some(f => normQ(f).indexOf(q) !== -1);
const countPkgs = (q) => TOURS.filter(t => t.type === 'package' && matchQ(q, [t.name, t.desc, t.sub])).length;
const countBeaches = (q) => BEACHES.filter(b => matchQ(q, [b.name, b.zone, b.desc])).length;
const countTransfers = (q) => TRANSFERS.filter(tf => matchQ(q, [tf.name, tf.origin, tf.destination, tf.desc])).length;
...
// ── Render: bloques zig-zag (imágenes SOLO URL válida) ──────────────────────
function renderServices() {
  ...
  w.innerHTML = SERVICES.map((s, i) =>
    ...
    '<img src="' + esc(safeImg(s.img)) + '" class="transfer-img" data-service-img="' + i + '" style="object-position:' + esc(s.imgPosition || 'center') + ';object-fit:' + esc(s.imgFit || 'cover') + ';" onerror="this.style.display=\'none\'"><div class="transfer-img-fallback">🚐</div></div>' +
...
function renderAbout() {
  ...
  w.innerHTML = ABOUT_BLOCKS.map((s, i) =>
    ...
    '<img src="' + esc(safeImg(s.img)) + '" class="transfer-img" data-about-img="' + i + '" style="object-position:' + esc(s.imgPosition || 'center') + ';object-fit:' + esc(s.imgFit || 'cover') + ';" onerror="this.style.display=\'none\'"><div class="transfer-img-fallback">📸</div></div>' +
    '<div class="transfer-text glass-panel"><h3 style="color:' + (i % 2 !== 0 ? 'var(--primary-dark)' : 'var(--teal)') + ';margin-bottom:8px;">' + esc(s.title) + '</h3>' +
    (s.subtitle ? '<h4 style="color:var(--teal);margin-bottom:5px;">' + esc(s.subtitle) + '</h4>' : '') + '<p>' + esc(s.desc) + '</p></div></div>').join('');
}

// ── 🏖️ Render: catálogo de playas (CON FILTRO DE LUPA) ──────────────────────
function renderBeaches() {
  const w = $('beaches-scroller'); if (!w) return;
  const isEdit = document.body.classList.contains('edit-mode');
  const q = globalSearchQuery;
  const list = (q && !isEdit) ? BEACHES.filter(b => matchQ(q, [b.name, b.zone, b.desc])) : BEACHES;
  let html = list.map((b, i) =>
    ...
    '<div class="beach-img"><div class="beach-fallback">' + (b.emoji || '🏖️') + '</div><img src="' + esc(safeImg(b.img)) + '" alt="' + esc(b.name) + '" onerror="this.style.display=\'none\'"></div>' +
  ...
  buildDots($('beach-dots'), list.length + 1);
...
// ── 🚐 Render: carrusel de traslados (CON FILTRO DE LUPA) ───────────────────
function renderTransferDestinations() {
  ...
  const isEdit = document.body.classList.contains('edit-mode');
  const q = globalSearchQuery;
  const list = (q && !isEdit) ? TRANSFERS.filter(tf => matchQ(q, [tf.name, tf.origin, tf.destination, tf.desc])) : TRANSFERS;
  if (!list.length) { w.innerHTML = '<div class="empty-note">🔍 Sin traslados que coincidan con tu búsqueda.</div>'; return; }
  w.innerHTML = list.map((tf, i) =>
...
// ── Tarjetas y grids (imágenes SOLO URL válida) ─────────────────────────────
...
function renderFeatured() {
  ...
  TOURS.filter(t => t.featured).forEach(t => {
    ...
    const mi = safeImg((t.imgs && t.imgs.length) ? t.imgs[0] : t.img);
...
// ── 🔍 LUPA INTELIGENTE: input de búsqueda (paquetes + playas + traslados) ──
document.querySelectorAll('.tour-search-input').forEach(i => i.addEventListener('input', e => {
  globalSearchQuery = e.target.value.toLowerCase().trim();
  document.querySelectorAll('.tour-search-input').forEach(x => { if (x !== e.target) x.value = e.target.value; });
  renderToursPage();
  if (e.target.id === 'global-search-input') {
    const q = globalSearchQuery;
    const active = document.querySelector('.page-view.active');
    if (!q) {
      if (active && ['page-tours', 'page-beaches', 'page-services'].indexOf(active.id) !== -1) { closeAllPages(); updateNav('map'); }
      renderBeaches(); renderTransferDestinations();
      return;
    }
    const cp = countPkgs(q), cb = countBeaches(q), ct = countTransfers(q);
    let target = 'tours';
    if (cb > cp && cb >= ct) target = 'beaches';
    else if (ct > cp && ct > cb) target = 'services';
    if (!active || active.id !== 'page-' + target) openPage(target);
    else { renderBeaches(); renderTransferDestinations(); }
  }
}));
function renderToursPage() {
  const w = $('tours-content'); if (!w) return;
  const q = globalSearchQuery;
  const p = TOURS.filter(t => t.type === 'package' && matchQ(q, [t.name, t.desc, t.sub]));
  w.innerHTML = '<div class="card-grid">' + (p.map(packageCard).join('') || (q ? '<div class="empty-note">🔍 Sin paquetes que coincidan con tu búsqueda.</div>' : '<div class="empty-note">Aún no hay paquetes disponibles.</div>')) + '</div>';
  bindGridCards(w);
  const sc = w.querySelector('.card-grid');
  buildDots($('tours-dots'), sc ? sc.querySelectorAll('.grid-card').length : 0);
  bindScrollerDots(sc, $('tours-dots'), '.grid-card');
}
...
function refreshAll() {
  sanitizeUrls();
  renderFeatured(); renderGrids(); renderMapMarkers();
  if (typeof refreshSelects !== 'undefined') refreshSelects();
  refreshRatings(); renderServices(); renderAbout(); renderTransferDestinations(); renderBeaches();
  if (typeof refreshTransferSelects !== 'undefined') refreshTransferSelects();
  if (typeof refreshBeachSelects !== 'undefined') refreshBeachSelects();
  applyTexts(); observeCards();
}
...
function openPage(n) {
  closeAllPages();
  if (n === 'map') { updateNav('map'); setTimeout(() => map.invalidateSize(), 300); return; }
  hidePlacePins();
  const p = $('page-' + n); if (p) p.classList.add('active');
  updateNav(n);
  if (n === 'reviews') renderReviews();
  if (n === 'services') { renderServices(); renderTransferDestinations(); }
  if (n === 'about') renderAbout();
  if (n === 'beaches') renderBeaches();
  applyTexts(); observeCards();
  if (mobileBtn && mobileBtn.classList.contains('active')) { mobileBtn.classList.remove('active'); navLinksObj.classList.remove('active'); }
}
...
// ✅ GALERÍA SIN RUTAS LOCALES: solo URLs válidas; si no hay → emoji + degradado
function getGallery(t) {
  let g = [], c = [];
  if (t.type === 'package' && t.places) {
    if (t.imgs) t.imgs.forEach((im, i) => { const s = safeImg(im); if (s) { g.push(s); c.push('📷 ' + t.name + ' · Foto ' + (i + 1)); } });
    t.places.forEach(pid => { const p = byId(pid); if (!p) return; const im = safeImg((p.imgs && p.imgs[0]) ? p.imgs[0] : p.img); if (im && g.indexOf(im) === -1) { g.push(im); c.push('📷 ' + p.emoji + ' ' + p.name); } });
  } else {
    if (t.imgs && t.imgs.length) t.imgs.forEach((im, i) => { const s = safeImg(im); if (s) { g.push(s); c.push('📷 ' + t.name + ' · Foto ' + (i + 1)); } });
    else if (safeImg(t.img)) { g.push(safeImg(t.img)); c.push('📷 ' + t.name); }
  }
  if (!g.length) { g.push(''); c.push('📷 ' + (t.name || '')); }
  return { g, c };
}
...
function openMedia(id, parentPkgId) {
  ...
  mTrack.innerHTML = gg.g.map((s, i) => '<div class="media-slide cf-' + (t.id.indexOf('custom') !== -1 ? 'custom' : t.id) + '"><div class="card-fallback" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:0;">' + (t.emoji || '🏖️') + '</div><img src="' + s + '" alt="' + esc(gg.c[i]) + '" class="' + (s ? '' : 'missing') + '" style="position:relative;z-index:1;" onload="if(!this.classList.contains(\'missing\'))this.previousElementSibling.style.display=\'none\'" onerror="this.classList.add(\'missing\');this.previousElementSibling.style.display=\'flex\'"><div class="ph-cap" style="z-index:2;">' + esc(gg.c[i]) + '</div></div>').join('');
...
// ✅ VIDEO: solo URL completa (bloquea rutas locales)
function openVideoModal() {
  const t = currentTour;
  if (!t || !t.video) { alert('Este tour aún no tiene video.'); return; }
  if (!isFullUrl(t.video)) { alert('⚠️ El video debe ser una URL completa (https://...).'); return; }
  const p = parseVideoUrl(t.video); const vc = $('video-modal-container');
  const safeVideoUrl = t.video.trim().toLowerCase().startsWith('javascript:') ? '#' : esc(t.video);
  if (p && p.type === 'iframe') vc.innerHTML = '<iframe src="' + p.embed + '" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe>';
  else if (p && p.type === 'direct') vc.innerHTML = '<video src="' + p.url + '" controls autoplay></video>';
  else vc.innerHTML = '<div class="video-fallback"><div style="font-size:3rem">🎥</div><p>No se puede incrustar este video.</p><a class="btn-primary" target="_blank" href="' + safeVideoUrl + '">Ver en la red social ↗</a></div>';
...
// ── 🔍 Búsqueda global (lupita del nav) — abrir/cerrar barra ────────────────
(function () {
...
// ── 📱 Flechas del carrusel de paquetes (móvil) ─────────────────────────────
