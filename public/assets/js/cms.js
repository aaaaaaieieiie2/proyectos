...
on($('btn-save-cms'), 'click', () => {
 ...
 TRANSFERS.push({ id: 'tf-custom-' + Date.now(), name, origin: $('tf-new-origin').value.trim() || 'Aeropuerto / Hotel', destination: dest, price_one_way: parseFloat($('tf-new-oneway').value) || 0, price_round_trip: parseFloat($('tf-new-roundtrip').value) || 0, max_passengers: parseInt($('tf-new-maxpax').value, 10) || 8, emoji: $('tf-new-emoji').value.trim() || '🗺️, price_basis: $('tf-new-basis') ? $('tf-new-basis').value : 'per_vehicle', desc: $('tf-new-desc').value.trim() || '', coords: [] });
